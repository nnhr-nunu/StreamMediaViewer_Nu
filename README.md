...
4. 必要に応じて「手動ぼかし」で足します。ONのとき戻す・太さ・クリアが出ます。
5. ✨でコントラストと彩度の自動補正を切り替えます。押すたびに「オフ → 弱 → 強」と変わります。ぼかしの強さ・言語・下のフォルダを読むか・待機画像は⚙から変えられます。
...
- 顔のぼかしは初期設定で有効です。強さの初期は約 80 です。すでに弱い強さで保存してある場合は⚙で上げてください。
- 車の番号や名札の自動ぼかし機能は、必要に応じてONにしてください。外れやすいので、配信前に手元で確認し、足りなければ四角や手動ぼかしで足してください。
...
4. Add **Manual blur** where needed. Undo, thickness, and Clear appear while it is on.
5. Click ✨ to cycle contrast and saturation enhancement: Off → Soft → Strong. Use ⚙ for blur strength, language, subfolders, and the standby image.
...
- Face blur is on by default. Strength starts around 80. If an older settings file kept a weaker value, raise it in ⚙.
- Turn on automatic plate / name-tag blur when needed. It can miss; review in the operator window and add a box or manual blur before sending.
